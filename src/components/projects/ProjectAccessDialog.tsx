...
export default function ProjectAccessDialog({ open, onOpenChange, project, onApproved }: ProjectAccessDialogProps) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const { data, error } = await supabase.rpc("project_access_state", { _viewer_id: user?.id ?? null, _project_id: project.id });
  ...
  const requestAccess = async () => {
    ...
    setSubmitting(true);
    const { error } = await supabase.from("project_access_requests").insert({ project_id: project.id, owner_user_id: project.owner_user_id, requester_id: user.id, message: message.trim() || null, nda_accepted: Boolean(project.nda_required && ndaAccepted), nda_accepted_at: project.nda_required && ndaAccepted ? new Date().toISOString() : null });
    setSubmitting(false);
