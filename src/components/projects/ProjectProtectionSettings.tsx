...
export default function ProjectProtectionSettings({ projectId, projectTitle, ownerUserId }: Props) {
  ...
  const load = async () => {
    ...
    const [{ data: security }, { data: access }] = await Promise.all([
      (supabase as any).from("project_security").select("visibility,nda_required").eq("project_id", projectId).maybeSingle(),
      (supabase as any).from("project_access_requests").select("id,requester_id,message,status,nda_accepted,created_at").eq("project_id", projectId).order("created_at", { ascending: false }),
    ]);
  ...
  const save = async () => {
    setSaving(true);
    const { error } = await (supabase as any).from("project_security").upsert({ project_id: projectId, owner_user_id: ownerUserId, visibility, nda_required: ndaRequired }, { onConflict: "project_id" });
    setSaving(false);
  ...
  const review = async (id: string, status: "approved" | "rejected") => {
    const { error } = await (supabase as any).from("project_access_requests").update({ status, reviewed_by: ownerUserId, reviewed_at: new Date().toISOString() }).eq("id", id).eq("owner_user_id", ownerUserId);
